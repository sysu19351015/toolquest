...
测试包含领域状态机、幂等与版本冲突、run 隔离、重启发现与恢复、损坏状态拒绝、
篡改检测重放、报告脱敏、MCP 工具契约，以及隔离状态目录的真实 stdio 子进程通信。
...
当前版本包含两个内置房间、十一个 MCP 工具、原子本地运行持久化、重启发现与
恢复、确定性事件重放、脱敏 Markdown 报告、JSONL 轨迹和按房间校准的评分。一个
状态目录只支持一个服务进程。远程 HTTP、鉴权、回放 UI、社区房间、多进程事务和
公开排行榜暂不包含。
